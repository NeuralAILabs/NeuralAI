    ...
    <div className="lg:col-span-2">
      <Link href="#home" className="flex items-center gap-2.5">
        <span className="text-[#b0421a]">
          <svg viewBox="0 0 64 64" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="h-9 w-9">
        ...
        <span className="text-xl font-semibold tracking-tight">Neural<span className="text-[#b0421a]"> AI</span></span>
      </Link>
      ...
      <form className="mt-6 flex w-full max-w-sm gap-2">
        <input type="email" placeholder="Your email" className="flex-1 rounded-full border border-slate-300 bg-white px-5 py-2.5 text-sm focus:border-slate-500 focus:outline-none" />
        <button type="submit" className="ui-btn-dark text-sm py-2.5 px-5">Subscribe</button>
      </form>
    </div>
  );
}
...
function FooterColumns() {
  return cols.map((col) => (
    <div key={col.title}>
      <h4 className="text-xs font-medium uppercase tracking-[0.12em] text-slate-500">{col.title}</h4>
      <ul className="mt-4 space-y-2.5">
        {col.links.map((l) => (
          <li key={l.label}>
            <Link href={l.href} className="text-sm text-[#1f1410]/80 hover:text-[#1f1410]">{l.label}</Link>
          </li>
        ))}
      </ul>
    </div>
    ...
    <footer className="ui-poppins bg-white text-[#1f1410] border-t border-slate-200">
      ...
      <div className="border-t border-slate-200">
        <div className="px-4 md:px-16 lg:px-24 xl:px-32 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs text-[#1f1410]/60">&copy; {year} NeuralAI Pvt. Ltd. All rights reserved.</p>
          <div className="flex items-center gap-5 text-xs text-[#1f1410]/60">
            <Link href="#" className="hover:text-[#1f1410]">Privacy</Link>
            <Link href="#" className="hover:text-[#1f1410]">Terms</Link>
            <Link href="#" className="hover:text-[#1f1410]">Cookies</Link>
          </div>
        </div>
      </div>
    </footer>
